...
##si esta prendido, el primer toque rebota normal y recien despues aplica el estado; la bola rompe unicamente ovillos que ya tenian el estado
...
func impactar_con_objeto(pelotita : BolaDePelos, objeto_a_impactar : Node2D):
	...
	if romper_solo_si_ya_lo_tenia and objeto_a_impactar is Ovillo and aplica_a(objeto_a_impactar):
		tenia = objeto_a_impactar.tiene_estado(estado.nombre)
	rebote_simple(pelotita, objeto_a_impactar, tenia)
	if not tenia and is_instance_valid(objeto_a_impactar):
		aplicar(objeto_a_impactar)


func al_impactar_ovillo(pelotita : BolaDePelos, ovillo : Ovillo):
	if not romper_solo_si_ya_lo_tenia:
		aplicar(ovillo)


func aplicar(ovillo : Ovillo) -> void:
